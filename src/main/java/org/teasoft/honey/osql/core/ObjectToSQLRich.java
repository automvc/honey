...
//			t_sql = t_sql + "[index0]";  //index0 不带,与单条共用.
			...
			for (int i = 0; i < len; i++) { // i=1
				...
				if (i == 0) {
					...
					_ObjectToSQLHelper._toInsertSQL_for_ValueList(sql_i, entity[i], excludeFieldList); // i
																										// 默认包含null和空字符串.因为要用统一的sql作批处理
...
//			}
			...
			for (int i = 0; i < len; i++) { // i=1
				...
				if (i == 0) {
					...
					oneRecoreList = _ObjectToSQLHelper._toInsertSQL_for_ValueList(sql_i, entity[i], excludeFieldList); // i
																														// 默认包含null和空字符串.因为要用统一的sql作批处理
					// t_sql = wrap.getSql(); // 每个sql不一定一样,因为设值不一样,有些字段不用转换. 不采用;因为不利于批处理
				}
...
//					t_sql +"  [Batch:"+ (i/batchSize) + index3  // 用于批量插入时设置值
	...
	private SqlValueWrap toDeleteByIdSQL0(Class c) {
		...
		sqlBuffer.append(K.delete).append(" ").append(K.from).append(" ").append(tableName).append(" ").append(K.where)
				.append(" ");
		;
...
//		String whereFields[] = whereFieldList.split(",");
		...
		if (condition == null || condition.getIncludeType() == null) {
			...
			return _ObjectToSQLHelper._toUpdateBySQL(entity, whereFields, condition.getIncludeType().getValue(),
					condition);
		}
	}
...
//			throw new ObjSQLException("ObjSQLException:updateFieldList at least include one field.");
		...
		if (condition == null || condition.getIncludeType() == null) {
			...
			return _ObjectToSQLHelper._toUpdateSQL(entity, updateFields, condition.getIncludeType().getValue(),
					condition);
		}
	}
