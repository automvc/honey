...
//		不经过MoreTable,直接传入sql,需要重新生成结构  V1.11
		...
		try {
			...
			List<Map<String, String>> rsMapList = getBeeSqlForApp().selectMapListWithColumnName(exe_sql,
					toStringArray(sql));
...
//							v = rs.getObject(tableName + "."+ _toColumnName(field[i].getName(), entity.getClass()));
							...
							tempValue = rsMap.get(tempCName);
							if (tempValue == null) tempValue = _toColumnName(field[i].getName(), entity.getClass()); // V2.1.8
																														// 避免自定义count(*)
																														// para时,没有带表名
						}
