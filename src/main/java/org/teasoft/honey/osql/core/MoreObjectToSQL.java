...
public class MoreObjectToSQL implements MoreObjToSQL {
	...
	public <T> String toSelectSQL(T entity) {
		return _MoreObjectToSQLHelper3._toSelectSQL(entity); // 默认过滤NULL和空字符串
	}

	@Override
	public <T> String toSelectSQL(T entity, int start, int size) {
		return _MoreObjectToSQLHelper3._toSelectSQL(entity, start, size); // 默认过滤NULL和空字符串
	}

	@Override
	public <T> String toSelectSQL(T entity, Condition condition) {
		return _MoreObjectToSQLHelper3._toSelectSQL(entity, condition); // 若condition没有设置IncludeType,默认过滤NULL和空字符串
	}
