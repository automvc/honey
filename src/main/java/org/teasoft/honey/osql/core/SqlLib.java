	...
	private int batchForMysql(String sql[], int batchSize) {
		...
		try {
			...
			if (len <= batchSize) {
				...
				for (int i = 0; i < len / batchSize; i++) {
					temp = _batchForMysql(sql[0], i * batchSize, (i + 1) * batchSize, conn, pst, batchSize,
							batchExeSql[1]);
					total += temp;
//					pst.clearBatch();  //clear Batch
...
//			要是原来就是自动提交,报异常, 也要关
	...
	private boolean isConstraint(SQLException e) {
		...
		return ("MySQLIntegrityConstraintViolationException".equals(className) // mysql
				...
				|| (e instanceof SQLIntegrityConstraintViolationException) || e.getMessage().contains("ORA-00001:") // Oracle
				...
				|| e.getMessage().contains("SQLITE_CONSTRAINT_PRIMARYKEY")
				|| e.getMessage().contains("PRIMARY KEY constraint") // SQLite
