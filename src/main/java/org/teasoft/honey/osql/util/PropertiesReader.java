...
//			if (fileName!=null && !fileName.trim().startsWith(File.separator)) fileName = File.separator + fileName.trim();
			...
			if ("bee.properties".equals(fileName)) Logger.debug(
					"Since V1.17 the jar of honey donot contain bee.properties file, if you donot create a new one , can ignore this message!");
			else Logger.debug(
