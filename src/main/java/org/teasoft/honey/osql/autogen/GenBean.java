...
//				String requiredStr=", required = true";
		...
		try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(entityFile)));) {
			...
			if (HoneyUtil.isCassandra())
				bw.write("//@Table(\"" + table.getSchema() + "." + tableName + "\")" + LINE_SEPARATOR);
			...
			if (config.isGenSwagger())
				bw.write("@ApiModel(description = \"#VALUE#\") ".replace("#VALUE#", tableComment) + LINE_SEPARATOR);
			...
			if (config.isGenSerializable()) {
				bw.write("\tprivate static final long serialVersionUID = " + HoneyUtil.genSerializableNum() + ";"
						+ LINE_SEPARATOR);
...
//		String selfName=entityName;
		...
		if (config.isGenComment() && !"".equals(tableComment))
			authorComment += " * " + tableComment + " (relative field name for Javabean " + entityName + ")"
					+ LINE_SEPARATOR;
		else
...
//		}
		...
		try {
			...
			if (e.getMessage().contains("You have an error in your SQL syntax;")
					&& e.getMessage().contains("where 1<>1")) {
	...
	private void initComment(Table table, Connection con) throws SQLException {
		...
		if (t_sql != null) {
			...
			sql = "select column_name,column_comment from information_schema.COLUMNS where TABLE_SCHEMA='"
					+ table.getSchema() + "' and TABLE_NAME=?"; // the first select column is column name, second is comment
		...
		try {
			...
			if (t_sql2 != null) {
				...
				sql2 = "select TABLE_NAME,TABLE_COMMENT from information_schema.TABLES where TABLE_SCHEMA='"
						+ table.getSchema() + "' and TABLE_NAME=?";
			} else if (DatabaseConst.ORACLE.equalsIgnoreCase(dbName)) {
