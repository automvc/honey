...
public class GenBean {
	...
	private void genBeanFile(Table table) {
		...
		for (int i=0; i < columnNames.size(); i++) {
			...
			getsetStr+="\t" + "public " + javaType + " " + getOrIs + getsetProNameStr + "() {" + LINE_SEPARATOR + "\t\t" + "return "
					+ propertyName + ";" + LINE_SEPARATOR + "\t}" + LINE_SEPARATOR + LINE_SEPARATOR +
...
//			bw.close();
	...
	private List<Table> getAllTables() {
		...
		if (!"".equals(config.getQueryTableSql().trim())) {
			...
			throw new BeeException(
					"There are not default sql, please check the bee.databaseName in bee.properties is right or not, or define queryTableSql in GenConfig!");
		}
		//			con = getConnection();
	...
	private void initComment(Table table, Connection con) throws SQLException {
		...
		if (t_sql != null)
			...
			throw new BeeException(
					"There are not default sql, please check the bee.databaseName in bee.properties is right or not, or define queryColumnCommnetSql in GenConfig!");
		}
		PreparedStatement ps=null;
		...
		try {
			...
			if (t_sql2 != null)
				...
				throw new BeeException(
						"There are not default sql, please check the bee.databaseName in bee.properties is right or not, or define queryTableCommnetSql in GenConfig!");
			}
